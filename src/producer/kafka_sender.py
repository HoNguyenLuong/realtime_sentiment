...
def send_comments(video_id, comments_data):
    ...
    for idx, comment in enumerate(comments_data):
        try:
            ...
            logger.debug(f"[send_comments] Uploading comment {idx} to MinIO as {object_name}, size: {len(comment_bytes)} bytes")
            ...
            try:
                ...
                logger.info(f"[send_comments] Successfully uploaded comment {idx} for video {video_id} to MinIO as {object_name}")
                success_count += 1
    ...
    logger.info(f"[send_comments] Finished processing: {success_count} comments uploaded, {error_count} errors for video {video_id}")
